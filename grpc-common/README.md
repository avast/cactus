...
you can choose whether you work with [`Task`](https://monix.io/docs/3x/eval/task.html), `Future` or something else. The only thing
you have to do is to provide an implicit instance of `FromTask[F]` (alias for `cats.arrow.FunctionK[Task, ?]`) or `ToTask[F]`
(alias for `cats.arrow.FunctionK[?, Task]`) for your _`F`_.
