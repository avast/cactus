...
object CactusMacros {
  ...
  def convertGpbToCaseClassT[A: c.WeakTypeTag](c: whitebox.Context): c.Expr[Either[CactusFailure, A]] = {
    ...
    c.Expr[Either[CactusFailure, A]] {
      q""" {
          ...
          try {
            Right(${caseClassConverter(c)(tpe, q" ${TermName("gpb")} ")})
          } catch {
  ...
  private def caseClassConverter(c: whitebox.Context)(tpe: c.universe.Type, gpb: c.Tree): c.Tree = {
    import c.universe._
  ...
  private def processEndType(c: whitebox.Context)
    ...
    resultType.toString match {
      ...
      case t if typeSymbol.isClass && typeSymbol.asClass.isCaseClass => // case class
        q" if ($query) ${caseClassConverter(c)(returnType, q"$getter ")} else throw CactusException(MissingFieldFailure(${name.toString})) "


      case t if typeSymbol.isClass && typeSymbol.asClass.baseClasses.map(_.name.toString).contains("TraversableLike") => // collection
        q" if ($query) $getter.asScala.toList else throw CactusException(MissingFieldFailure(${name.toString})) "
