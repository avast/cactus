...
object CactusMacros {
  ...
  implicit def CollAToCollB[A, B, T[X] <: TraversableLike[X, T[X]]](coll: T[A])(implicit cbf: CanBuildFrom[T[A], B, T[B]], aToBConverter: Convert[A, B]): T[B] = {
    coll.map(aToBConverter.apply)
  }

  implicit def TryAToTryB[A, B](ta: Try[A])(implicit aToBConverter: Convert[A, B]): Try[B] = {
    ta.map(aToBConverter.apply)
  }

  implicit def OptAToOptB[A, B](ta: Option[A])(implicit aToBConverter: Convert[A, B]): Option[B] = {
    ta.map(aToBConverter.apply)
  }

  implicit def AToB[A, B](a: A)(implicit aToBConverter: Convert[A, B]): B = {
